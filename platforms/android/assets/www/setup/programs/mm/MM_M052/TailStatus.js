angular.module('WingsMobileStarter').controller('MM_M052.TailStatus', [
    '$scope',
    '$state',
    ...
    '$ionicLoading',
    'md5',
    function($scope,$state,WingsRemoteDbService,WingsUtil,WingsTransactionDBService,WingsDialogService,$ionicSlideBoxDelegate,$ionicHistory,$ionicTabsDelegate,WingsPouchDbSetupService,$ionicBackdrop,$ionicPopover,$timeout,$ionicModal,sy,$ionicLoading,md5) {
        console.log("MM_M052 Tail Status");
        var defaultTailNumber = $rootScope.MM_0052_defaultTailNumber;
        ...
        ////    EVENTS    ////
        var onflightcreate = $rootScope.$on('onflightcreate', function(){
        	$timeout(function() {
                $scope.SYNC();
            },100);
        });
      
        var refreshFlightList = $rootScope.$on('refreshFlightList', function(){
    	    $timeout(function() {
    		    $scope.getFlightList();
          },100);
        });
        var onDiscrepancyCreate = $rootScope.$on('onDiscrepancyCreate', function(){
    	    $timeout(function() {
    		    pushDefect();
          },100);
        });
        var packageListener = $rootScope.$on('onAircraftStatusFinished', function(){
  	        $timeout(function() {
  	    	    $scope.getPackages();
            },100);
        });
        ...
        $scope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams, error) {
        	if (toState.name == 'app.MM_M052') {
	      		$('#m052').siblings('.upper-canvas').remove();
	          	$('#m052').parent('.canvas-container').before($('#m052'));
        ...
        $scope.addMoreDefects = function(done) {
        ...
        $scope.switchSlider = function (index) {
            if ($scope.activeSlideIndex != index -1){
                $scope.ischangeOnClick = true;
            }
            $scope.activeSlideIndex = index-1;
            $scope.slider.slideTo(index-1, 200);
            $ionicTabsDelegate.select(index-1, false);
        };
        ...
        $rootScope.$ionicGoBack = function() {
        	if ($state.current.data.programId == "MM_M052_TAILSTATUS") {
        ...
        $scope.getFlightList = function () {
            ...
            var sql =" Select * from (                                                                                      " +
                     ...
                     "Order By SCHEDULED_OFF_BLOCK ASC )  limit 5" +
                     ")                                                " +
                     " UNION ALL                                                                                            " +
                     " Select * from (                                                                                      " +
                     " Select b.ID,                                                                                         " +
    			...
    			console.log('READ Start'+moment().format('DD-MM-YYYY HH:mm:ss'));

            WingsTransactionDBService.executeSql(sql,[$rootScope.globals.currentUser.divNo,defaultTailNumber,$rootScope.globals.currentUser.divNo,defaultTailNumber]).then(function (result) {
        		console.log('READ End'+moment().format('DD-MM-YYYY HH:mm:ss'));
    	...
    	$scope.showFlightDetails = function (object) {
            ...
            if (object.TABLE_NAME == 'MM_FLIGHTS') {
                   $rootScope.MM_M052_id = object.ID;
                   $state.go('app.MM_0052_FlightDetails')
            } else {
        ...
        // Teknisyen flight modify etme kaldirildi.
        /*$scope.modifyFlight = function (object) {
        	...
        	if (object.TABLE_NAME == 'MM_FLIGHTS') {
                ...
                var buttonArray= ['Ok','Cancel'];
                WingsDialogService.confirm('Flight process will be started.Do you want to continue ?','Confirm',buttonArray).then(function(buttonIndex) {
                    var btnIndex = buttonIndex;
                    if (buttonIndex == 1) {
                        $rootScope.MM_M051_id = object.ID;
                        pushOnlyFlight(object.MOBILE_RECORD_ID).then(function (result) {
        ...
        $scope.openFlight = function (object) {
            $rootScope.MM_M051_viewMode ='logInsert';
            $state.go('app.MM_0051_Flight');
            $scope.popoverMenu.hide();
            $scope.cancelSelect();
        };
    */
	    ...
	    $scope.getDiscrepancies = function (condition) {
            ...
            var sql = " Select *                                             " +
                      ...
                      "    And a.Tail_Number = ?                             " +whereCondition+
                      "Order by Report_Date desc                             " +
                      " Limit 250                                            "; 
            var parameters = [$rootScope.globals.currentUser.divNo,defaultTailNumber];
       ...
       // $scope.getDiscrepancies();
        $scope.newDiscrepancy = function () {
            ...
            $rootScope.MM_M055_TailNumber =  $rootScope.MM_0052_defaultTailNumber;
            $rootScope.MM_M055_Station = '';
            $rootScope.MM_M055_ControlNumber = '';
            if (!WingsUtil.IsNull($rootScope.MM_0052_defaultStation)) {
            	$rootScope.MM_M055_Stations = [$rootScope.MM_0052_defaultStation];
            } else {
            	$rootScope.MM_M055_Stations = null;
            }
            //$rootScope.MM_M052_AircraftType = $scope.flight.aircraftType;
	    ...
	    $scope.getPackages = function () {
	    	 $scope.packages = [];
	         var sql = "Select ifnull(nullif(Planned_Date,''),'9999-99-99')  Order_By,                  " +
	         		   "       Discrepancy_Number Work_Pack_Number,                                     " +
			           "       Planned_Date Planned_Date,                                               " +
			           "       Status Status,                                                           " +
			           "       Order_Number Order_Number,                                               " +
			           "       Discrepancy Description,                                                 " +
			           "       'WORK_ORDER' Table_Name,                                                 " +
			           "       'false' Department_Line_Flag,                                            " +
			           "       Mobile_Record_Status Mobile_Record_Status,                               " +
			           ...
			           "   And a.Planned_Date < date('now','+1 day')                                    " +
			           "   And a.Order_Flag = 'Y'                                                       " +
			           " Union All                                                                      " +
			           "Select ifnull(nullif(Planned_Start_Date,''),'9999-99-99')  Order_By,            " +
			           "       Package_Number Work_Pack_Number,                                         " +
			           "       Planned_Start_Date Planned_Date,                                         " +
			           "       Tracking_Status Status,                                                                  " +
			           "       Work_Order_Number,                                                       " +
			           "       '<b style=\"font-size: 16px;color: #565656;\">'||Package_Type ||'</b> | '|| Internal_Comment Description, " +
			           "       'PACKAGES' Table_Name,                                                   " +
			           ...
			           "   And a.Tail_Number = ?                                                        " +
			           "   And a.Status In ('OPEN')                                                     " +
			           "   And ifnull(a.Tracking_Status,'X') != 'COMPLETED'                             " ;
			           //"   And date(a.Planned_Start_Date) < date('now','+1 day')                              ";
			       /*    " Union All                                                                      " +
			           "Select  ifnull(nullif(datetime(Daily_Last_Done,'+'||Daily_Interval||' hour'),''),'9999-99-99')  Order_By, " +
			           "        Daily_Last_Done_Station Work_Pack_Number,                               " +
			           "        datetime(Daily_Last_Done,'+'||Daily_Interval||' hour') Planned_Date,    " +
			           "        'OPEN' Status,                                                          " +
			           "        '' Work_Order_Number,                                                   " +
			           "        'DAILY CHECK ACCOMPLISHED AT ' Description,                             " +
			           "        'NOTES' Table_Name,                                                     " +
			           "        'Y' Department_Line_Flag,                                               " +
			           "        Mobile_Record_Status,                                                   " +
			           "        Mobile_Record_Id,                                                       " +
			           "        Daily_Last_Done,                                                        " +
			           "       '' Id,                                                                   " +
			           "       'DAILY' Type                                                             " +
			           "   From Mm_Aircraft_Status a                                                    " +
			           "  Where a.Div_No = ?                                                            " +
			           "    And a.Tail_Number = ?                                                       " +
			           "    And a.Notes_Active = 'Y'                                                    " +
			           "  Order By 1                                                                    " ;*/
	
	         WingsTransactionDBService.executeSql(sql,[$rootScope.globals.currentUser.divNo,defaultTailNumber,$rootScope.globals.currentUser.divNo,defaultTailNumber]).then(function (result) {
	             if (result.length > 0) {
	            	 for (i in result) {
	             		if (!WingsUtil.IsNull(result[i].Planned_Date))
	             			result[i].Planned_Date = new Date(moment(result[i].Planned_Date));
	             		/*if (result[i].Table_Name == 'NOTES') {
	             			result[i].Work_Pack_Number = result[i].Work_Pack_Number+' - '+moment(result[i].Daily_Last_Done).format('DD-MMM HH:mm');
	             			result[i].Description = result[i].Description+moment(result[i].Daily_Last_Done).format('DD-MMM-YYYY HH:mm');
	             		}*/
	             		result[i].selected_flag = false;
	            	 }
	             }
	    ...
	    $scope.closePackageModal = function () {
	    	$scope.packageModal = {
	    		tailNumber     : defaultTailNumber,
	    	    employeeNumber : $rootScope.globals.currentUser.userNumber,
	    	    type           : $scope.selectedPackage.Type,
	    	    station        : $rootScope.MM_0052_defaultStation,
	    	    closeDate      : '',
	    	    closedBy       : $rootScope.globals.currentUser.userName
	    	};
	        $scope.modal.show();
	        $timeout(function() {
	        }, 250);  
	    ...
	    $scope.selectPackage = function(pkg) {
	    	if (pkg.Table_Name == 'WORK_ORDER') {
	    		$rootScope.MM_M055_Discrepancy_id = pkg.Mobile_Record_Id;
	            $rootScope.MM_M055_TailNumber = defaultTailNumber;
	            $rootScope.MM_M055_FromState = 'Package';
	            $rootScope.MM_M055_Station = [$rootScope.MM_0052_defaultStation];
	            $state.go('app.MM_M055');
	    	} else {
	    		if ($scope.selectedPackage == pkg) {
	    			$scope.cancelSelect();
	    		} else {
	    			$scope.cancelSelect();
	    			pkg.selected_flag = true;
	    			$scope.selectedPackage = pkg;
	    		}
	    	}
	    };
	    $scope.openWorkcard = function(object) {
	    	if (object.Table_Name == 'WORK_ORDER') {
	    		...
	    		$rootScope.PR_M108 = {};
	    		$rootScope.PR_M108.Project_Number = object.Work_Pack_Number;
	    		$state.go('app.PR_M108');
	    	}
	    };
	    $scope.cancelSelect = function() {
	        angular.forEach($scope.packages, function(o) {
	            o.selected_flag = false;
	        });
	        $scope.selectedPackage = '';
	    };
	
	    $scope.performPackage = function () {
	    	if (WingsUtil.IsNull($scope.packageModal.closeDate)) {
	            WingsDialogService.error("Perform Date is required.");
	            return false;
	    	}
	    	if ($scope.digitalSign != true) {
		    	...
		    	$scope.modal.hide();
		    	/*if ($scope.selectedPackage.Table_Name == 'NOTES') {
		    		var sql = new StoredFuncProcBuilder("Mb_Apps.Do_Package", 
		    				"i_Div_No",                      $rootScope.globals.currentUser.divNo,
		    				"i_Action",                      'CREATE',
		    				"i_Mobile_Package_Id",           $scope.selectedPackage.Mobile_Record_Id,
		    				"i_Tail_Number",                 defaultTailNumber,
		    				"i_Released_By_Employee_Number", $rootScope.globals.currentUser.userNumber,
		    				"i_Work_Order_Station", 	     "",
		    				"i_Start_Date ", 	             moment($scope.packageModal.startDate).format('YYYY-MM-DD HH:mm'),
		    				"i_Close_Date",                  moment($scope.packageModal.closeDate).format('YYYY-MM-DD HH:mm'),
		    				"i_Closed_By_Employee_Number",   $rootScope.globals.currentUser.userNumber,
		    				"o_Data",                        '');
					...
					WingsRemoteDbService.executeFunction(str).then (function (response) {
						$rootScope.$emit('onPackageCreate');
			    		$scope.getPackages();
			    	...
			    	var sql=" Update Mm_Packages                        " +
			    			"	 Set Work_Order_Station        = ?,     " +
			    			"        Actual_Start_Date         = ?,     " +
			    			"        Close_Date                = ?,     " +
			    			"        Closed_By_Employee_Number = ?,     " +
			    			"        Mobile_Record_Status      = ?      " +
			    			"  Where Mobile_Record_Id          = ?       ";
			    	var parameters = [$scope.packageModal.station,
			    		$scope.packageModal.startDate,
			            moment($scope.packageModal.closeDate).format('YYYY-MM-DD HH:mm'),
			    		$scope.packageModal.employeeNumber,
			    		'READY',
			    		$scope.selectedPackage.Mobile_Record_Id];
			    	WingsTransactionDBService.executeSql(sql,parameters).then(function (result) {
			    		$scope.getPackages();
			    		$scope.SYNC();
			            return deferred.resolve("GOHEAD");
			            ...
			            WingsDialogService.error(JSON.stringify(error));
			            console.log('Package Close Error'+JSON.stringify(error));
			            return deferred.reject("Package-Close-Error : " +JSON.stringify(error));
			        });
			        return deferred.promise;
			    //}
	    	}
	    };
	    $ionicModal.fromTemplateUrl ('templates/performPackage.html', {
	        scope: $scope
	    }).then(function(modal) {
	        $scope.modal = modal;
	    });
	    ...
	    $scope.showModal = function (action) {
	        ...
	        if (!WingsUtil.IsNull($scope.canvas)) {
	            ...
	            $scope.canvas = new fabric.Canvas ('m052');
	            ...
	            $scope.canvas.setHeight($scope.modal2.modalEl.clientHeight-200);
	            $scope.canvas.setWidth($scope.modal2.modalEl.clientWidth);
	            $scope.canvas.width  = $scope.modal2.modalEl.clientWidth;
	            $scope.canvas.height = ($scope.modal2.modalEl.clientHeight)-200;
        ...
        $scope.signin = function () {
        	...
        	sy.Login ($scope.user.username,$scope.user.password).then(function (response) {
        		...
        		if (response.success) {
	    			...
	    			$scope.performPackage();
	    			$scope.hideSignScreen();
        		} else {
	    ...
	    /*$scope.createNewPackage = function () {
	    	$scope.modal.hide();
	   	    var deferred = $q.defer();
	    	var sql=" Insert Into Mm_Packages (Div_No,Status,Tail_Number,Released_By_Employee_Number,Work_Order_Station,Actual_Start_Date,Close_Date,Closed_By_Employee_Number,Planned_Start_Date,Mobile_Record_Status) " +
			        " Values (?,?,?,?,?,?,?,?,?,?)";       
	    	var parameters = [ 
	    		$rootScope.globals.currentUser.divNo,
	    		'OPEN',
	    		$scope.newPackage.tailNumber,
	    		$scope.newPackage.employeeNumber,
	    		$scope.newPackage.station,
	    		WingsUtil.IsNull($scope.newPackage.startDate)?'': moment($scope.newPackage.startDate).format('YYYY-MM-DD HH:mm'),
	    		WingsUtil.IsNull($scope.newPackage.closeDate)?'': moment($scope.newPackage.closeDate).format('YYYY-MM-DD HH:mm'),
	    		$scope.newPackage.closedBy,
	    	    WingsUtil.IsNull($scope.newPackage.closeDate)?'': moment($scope.newPackage.closeDate).format('YYYY-MM-DD HH:mm'),
				'READY'];
	    	WingsTransactionDBService.executeSql(sql,parameters).then(function (result) {
	    		$scope.getPackages();
	            ...
	            WingsDialogService.error(JSON.stringify(error));
	            console.log('Package Insert Error'+JSON.stringify(error));
	            return deferred.reject("lof-Error : " +JSON.stringify(error));
	        });
	        return deferred.promise;
	    }*/
	    
	    /*$scope.createNewPackageModal = function () {
	    	$scope.newPackage = {
	    		tailNumber     : defaultTailNumber,
	    	    releaseBy      : $rootScope.globals.currentUser.userName,
	    	    employeeNumber : $rootScope.globals.currentUser.userNumber,
	    	    startdate      : '',
	    	    station        : $rootScope.MM_0052_defaultStation,
	    	    type           : 'DAILY',
	    	    closeDate      : '',
	    	    closedBy       : $rootScope.globals.currentUser.userNumber
	    	};
	        $scope.modal.show();
	        $timeout(function() {
	        }, 250);  
	    ...
	    function insertFlightConsumptions (consumptions) {
	        ...
	        for (i in consumptions) {
	            parameters = [consumptions[i].id,
	                          ...
	                          consumptions[i].consumption_flag=='N'?'false':'true',
		        ...
		        if (builders.length > 0) {
		             ...
		             WingsRemoteDbService.executeFunction(str).then (function (response) {
		            	 ...
		            	 if (result.isSuccess =='true'  && result.errorText == '') {
		                     ...
		                     $rootScope.MM_M052_id = angular.fromJson(result.result.o_Data).FLIGHT_ID;
		                     if (crews.length > 0) {
		                    	 var sql = new StoredFuncProcBuilder("Mb_Apps.Do_Flight",   
									                    			 "i_Div_No",             divNo,
									                    			 "i_Action",             'CREW',
									                    			 "i_Flight_Id",          $rootScope.MM_M052_id,
									                    			 "i_Crew_Types",         crews,
		                    	 ...
		                    	 WingsRemoteDbService.executeFunction(str).then (function (response) {
		                    		 ...
		                    		 if (result.isSuccess =='true'  && result.errorText == '') {
		                    			 WingsRemoteDbService.HandleFeedback(result);
		                    			 return deferred.resolve($rootScope.MM_M052_id);
		                    		 }
		                    	 }, function (error) {
