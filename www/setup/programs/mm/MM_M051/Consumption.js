angular.module('WingsMobileStarter').controller('MM_M051.Consumption', [
        '$scope','WingsSetupDBService','WingsTransactionDBService','$ionicHistory','WingsDialogService','WingsUtil','sy',
        function($scope,WingsSetupDBService,WingsTransactionDBService,$ionicHistory,WingsDialogService,WingsUtil,sy) {
            console.log("MM_M051.Consumption");
            $scope.lastConsumption = [{
                    date:'',
                    description:'',
                    amount:''
            }];
            var flightId = $rootScope.MM_M051_id;
            $scope.viewMode = 'insert';
            $scope.isConsShown = true;
            if(!WingsUtil.IsNull($rootScope.MM_M051_Consumption_id)) {
            ...
            sy.GetTableRows("Select * From Mm_Consumption_Types Where Div_No = ? And Active = 'Y' Order By Consumption_Type",[$rootScope.globals.currentUser.divNo]).then(function(result){
                $scope.fuelTypesLov = result;
            }); 
            ...
            function Query () {
                ...
                var parameters = [flightId,$rootScope.MM_M051_Consumption_id]
                WingsTransactionDBService.executeSql(sql,parameters).then(function (result){
                    console.log('MM_M051 Counsumptions:'+JSON.stringify(result));
                    ...
                    $scope.consumption.vendor.vendorName= result[0].VENDOR_NAME;
                    $scope.consumption.fuelType = result[0].CONSUMPTION_TYPE;
                    ...
                    console.log(result[0].CONSUMPTION_DATE);
                    $scope.consumption.date = new Date(moment(result[0].CONSUMPTION_DATE).format('DD MMMM YY, HH:mm'));
            ...
            $scope.Save = function () {
                if ($scope.consumption.noRefuellingFlag == false && (WingsUtil.IsNull($scope.consumption.date) || WingsUtil.IsNull($scope.consumption.fuelType) || WingsUtil.IsNull($scope.consumption.departure) || WingsUtil.IsNull($scope.consumption.vendor.vendorName))) {
                    WingsDialogService.error('Please fill columns !');
                    return false;
                }
                var sql=' Insert Into MM_FLIGHT_CONSUMPTIONS (Div_No,No_Refuelling_Flag,Vendor_Number,Vendor_Name,Fuel_Type,Density,Arrival,Added,Departure,Remaining,Consumption_Date,Flight_Id,Uom,Consumption_Type) '+
                        ' Values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
            ...
            $scope.Update = function () {
                ...
                var parameters = [  $rootScope.globals.currentUser.divNo,
                                    ...
                                    $scope.consumption.uom,
                                    flightId,
                                    $rootScope.MM_M051_Consumption_id];
                WingsTransactionDBService.executeSql(sql,parameters).then(function (result){
                        $rootScope.$emit('onConsumptionCreate');
                        $ionicHistory.goBack();
                        return deferred.resolve("GOHEAD");
